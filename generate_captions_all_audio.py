...
####################
# 250328-1320 Transcribing Audio Calls
...
# audio_dir = Path("videos")
audio_dir = Path("/Users/nic/Dropbox/Kaltura/audio")
# output_dir = Path("videos")
output_dir = Path("/Users/nic/Dropbox/Kaltura/audio")
output_dir.mkdir(exist_ok=True)

# # Process all video files in directory
# for audio_file in audio_dir.glob("*.mp4"):
...
for audio_file in audio_files:
    count += 1 
    transcript_pattern = f"{audio_file.stem}.srt"
    existing_transcripts = list(output_dir.glob(transcript_pattern))
    
    if existing_transcripts:
        if verbose:
            print(f"\n❌ #{count} skipping {audio_file.name} - transcript already exists\n")
        continue

    file_start = time.time()
    print(f"\n\n{datetime.now().strftime('%H:%M:%S')} =============== ℹ️  Processing {audio_file.name}...\n")
    
    # Transcribe and get both file paths
    srt_path, txt_path = generate_srt(str(audio_file))
    ...
    if verbose:
        print(f"Total processing time for {audio_file.name}: {round((time.time() - file_start)/60, 1)}min")
